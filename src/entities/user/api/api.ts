import { apiAuth } from '@/shared/api/instances';
...
export async function login(credentials: LoginRequestDTO): Promise<LoginResponseDTO> {
  const { data } = await apiAuth.post<LoginResponseDTO>('/login', credentials);
  return data;
}
...
export async function getCurrentUser(): Promise<GetCurrentUserResponseDTO> {
  const { data } = await apiAuth.get<GetCurrentUserResponseDTO>('/me');
  return data;
}
